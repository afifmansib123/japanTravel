...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log('📥 Query parameters:', { 
      category, isActive, isFeatured, difficulty, location, 
      minPrice, maxPrice, page, limit, sortBy, sortOrder, populate 
    });
    ...
    if (populate) {
      tourPackagesQuery = tourPackagesQuery.populate('category', 'name description slug');
    }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const inclusions = JSON.parse(formData.get('inclusions') as string || '[]') as string[];
    const exclusions = JSON.parse(formData.get('exclusions') as string || '[]') as string[];
    const isActive = formData.get('isActive') === 'true';
    const isFeatured = formData.get('isFeatured') === 'true';

    // 4. Create a temporary object for validation
    const body = { name, description, category, location, duration, price, maxGroupSize };

    // Validate required fields
    const requiredFields = ['name', 'description', 'category', 'location', 'duration', 'price', 'maxGroupSize'];
...
if (photos && photos.length > 0 && photos[0].size > 0) {
  ...
  console.log('No valid photos found to upload.');
    ...
    console.log('✅ Tour package saved successfully with ID:', savedTourPackage._id);
    ...
    if (error.code === 11000) {
      return NextResponse.json({ error: 'Tour package with this name already exists' }, { status: 409 });
    }
