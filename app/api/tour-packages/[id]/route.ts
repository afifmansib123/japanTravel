...
export async function PUT(
  ...
  try {
    const { id } = params;
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return NextResponse.json({ error: 'Invalid tour package ID format' }, { status: 400 });
    }
    ...
    const updateData: any = {
      ...
      discountedPrice: (body.discountedPrice !== undefined && body.discountedPrice !== null && body.discountedPrice !== '')
    ...
    ).populate('category', 'name description slug').lean();
    ...
    return NextResponse.json({ error: 'Failed to update tour package', details: error.message }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    console.log('✅ Tour package deleted successfully:', deletedTourPackage.name);
